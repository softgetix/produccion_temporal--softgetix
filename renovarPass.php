...
if(!$_POST){
    ...
    if(!$arrValid){
	$msgErr = $lang->localizart->recupero_contrasena->msg_error_link;
        $sinForm = true;	
    }
}
//-- --//
...
if($_POST){
    if(isset($_POST['txtPassword']) && isset($_POST['txtPassword_repeat'])){
        if($_POST['txtPassword'] == $_POST['txtPassword_repeat']){
           if(isset($_POST['txtPasswordActual'])){
                include "includes/conn.php";
                include "clases/clsUsuarios.php";
                $objUsuario = new Usuario($objSQLServer);
                
                if($objUsuario->loginPorId($idUsuario, $_POST['txtPasswordActual'])){
                    if(validarNuevaContrasenna($_POST['txtPassword'])){
                        if($_POST['txtPasswordActual'] != $_POST['txtPassword']){
                            ...
                            if(empty($msgErr)){
                                $msgOk = $lang->localizart->recupero_contrasena->msg_ok_actualizacion;
                                $sinForm = true;	
                                $irAlLogin = true;
                            }
                        }
                        else{
                            $msgErr =$lang->localizart->recupero_contrasena->msg_error_pass_iguales;
                        }
                    }
                    else{
                    ...
                    $msgErr = $lang->localizart->recupero_contrasena->msg_error_actual_password;
                }
           }
           else{
               $msgErr = $lang->localizart->recupero_contrasena->msg_campos_obligatorios;
           }
        }
        else{
...
<body style="overflow:auto;">
    ...
    <div id="contenido">
        <form name="myform" method="post" action="<?=$_SERVER['REQUEST_URI']?>"> 
            <h2>Renovaci&oacute;n de Contrase&ntilde;a <?php /*=$lang->localizart->recupero_contrasena->title_recupero*/ ?></h2>
            <?php if(!empty($msgErr)){?><p class="required_error"><?=$msgErr?></p><?php }elseif(!empty($msgOk)){?><p class="required_ok"><?=$msgOk?></p><?php }?>
            
            <?php if(!$sinForm && $_GET['ref']){?>
            <input type="hidden" name="idUsuario" value="<?=$idUsuario?>" />
            <input type="hidden" name="reset_code" value="<?=$reset_code?>" />
            <fieldset>
                <label><?=$lang->localizart->recupero_contrasena->password_actual?></label>
                <input type="password" name="txtPasswordActual" autocomplete="off"  />
            </fieldset>
            <fieldset>
