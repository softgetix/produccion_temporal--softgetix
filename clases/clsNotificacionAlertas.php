...
class NotificacionAlertas extends Nomenclador{
    ...
    function obtenerAlertasAgrupadas($alertas){
        ...
        if(is_array($res)){
            ...
            foreach($res as $id => $item){
                ...
                if ($id_reporte == '6'){
                    if(isset($res[$id]['nomenclado'])){
                        $res[$id]['nomenclado'] = NULL;
                    }
                }    
                elseif ($res[$id]['nomenclado'] == null){
   ...
   function obtenerAlertasDelAgenteNoConfirmadas($ultimoId = 0, $cantidadSelect = 100, $returnCantidad = false, $idevento = NULL){
		$seteado = false;
        ...
        if($idevento){
            $strSQL.= " AND ME.me_dr_id = ".(int)$idevento;
        }
