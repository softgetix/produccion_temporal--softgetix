...
class PHPExcel
{
	...
	function setFormatoRows($arralCol){
		foreach($arralCol as $item){
			$this->getActiveSheet()->getStyle($item.'1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$this->getActiveSheet()->getStyle($item.'1')->applyFromArray(array('fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID,'color' => array('rgb' => 'f7f7d5'))));
			$this->getActiveSheet()->getStyle($item.'1')->getFont()->setBold(true);
			$this->getActiveSheet()->getColumnDimension($item)->setAutoSize(true);
		}	
	}
	
	function alignCenter($alingCenterCol){
		foreach($alingCenterCol as $item){
			$this->getActiveSheet()->getStyle($item)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		}	
	}
	
	function alignLeft($alingCenterCol){
		foreach($alingCenterCol as $item){
			$this->getActiveSheet()->getStyle($item)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		}	
	}
