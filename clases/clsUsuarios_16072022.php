...
class Usuario extends Abm{	
	...
	function obtenerUsuarios($id=0,$filtro="", $nombreUsuario="", $empresa=0){
		...
		$sql="SELECT ".$selectTop." us_id, us_nombreUsuario,us_pasaporte,us_zona, us_nombre, us_apellido, us_mailContacto, us_mailAlertas, us_telefono, us_expira,us_cant_fallido,us_ultimo_acceso, us_acceso_fallido,	us_celular, us_nextel, us_pe_id, us_cl_id, us_usuarioCreador, us_fechaCreado, pe_nombre, cl_razonSocial,
	...
	function validarCambioPassword($datos){
		...
		if($reg = $this->objSQL->dbGetRow($rs,0,3)){
			if(
				strtotime('+30 minute',strtotime($reg['us_reset_date'])) >= strtotime(date('Y-m-d H:i'))//-- Si la fecha de solicitud de seteo es menor a 5min
