...
class Viajes extends Abm {
	...
	function getListadoViajes($filtros = NULL, $exportacion = false){
		...
		$sql.= " ,um_us_id, vi_contenedor"; //-- verifico si tiene movil asignado para visalizar mapa --//
		...
		//$sql.= " INNER JOIN tbl_usuarios_moviles WITH(NOLOCK) ON um_mo_id = vi.vi_mo_id AND um_us_id = ".(int)$_SESSION['idUsuario'];
...
//die ($sql);
	...
	function filtrosViajes($filtros){
		...
		$sql.= " and  vd_orden = case when vi_dador = 12481 then 1 else vd_orden end  ";
	...
	<tr id="n_<?=$datos['re_id']?>"><tr id="n_<?=$datos['re_id']?>">
        ...
        <?php }?>
        <td width="33%" style="vertical-align:middle;"><?=$datos['re_nombre']?></td>
		<?php if(tienePerfil(array(27,28))){?>
		<td width="60" style="vertical-align:middle;">
		<input type="text" name="pallets_stock_<?=$datos['re_id']?>" id="pallets_stock_<?=$datos['re_id']?>" value="<?=$datos['vd_stock']?>" class="only_number" disabled="true">
		</td>
        <?php }?>
        ...
        <td <?=(!tienePerfil(array(9,10,11,12,27,28))?'class="td-last"':'')?> style="vertical-align:middle;">
	...
	function getReferencias($datos = NULL){
		$sql = " SELECT vd_id, vd_re_id, re_nombre, vd_ini, vd_fin, vd_ini_real, vd_fin_real, vd_stock";
	...
	function setViajesDestinos($destinos){
		...
		foreach($destinos as $item){
			$sql_select = " SELECT top 1 re_nombre, vd_id, vd_ini, vd_fin, vd_orden, re_email, rc_latitud, rc_longitud, vd_stock ";
			$sql_select.= " FROM tbl_referencias  WITH(NOLOCK) ";
			$sql_select.= " INNER JOIN tbl_referencias_coordenadas  WITH(NOLOCK) ON rc_re_id = re_id ";
			...
			if($arr_vd['vd_id'] && !$this->insertDestino){
				if(date('d-m-Y H:i',strtotime($item['vd_ini'])) != date('d-m-Y H:i',strtotime($arr_vd['vd_ini'])) && !empty($item['vd_ini'])){
					$data_actual = $lang->system->fecha_inicio.'['.formatearFecha($arr_vd['vd_ini']).']';	
					$data_update = $lang->system->fecha_inicio.'['.formatearFecha($item['vd_ini']).']';
					$coma = ', ';
				}				
				if(date('d-m-Y H:i',strtotime($item['vd_fin'])) != date('d-m-Y H:i',strtotime($arr_vd['vd_fin'])) && !empty($item['vd_fin'])){
					$data_actual = $coma.$lang->system->fecha_fin.'['.formatearFecha($arr_vd['vd_fin']).']';	
					$data_update = $coma.$lang->system->fecha_fin.'['.formatearFecha($item['vd_fin']).']';
					$coma = ', ';
				}
				if($arr_vd['vd_orden'] != (int)$item['vd_orden']){
					$data_actual = $coma.$lang->system->orden.'['.$arr_vd['vd_orden'].']';	
					$data_update = $coma.$lang->system->orden.'['.(int)$item['vd_orden'].']';
				}
				if($arr_vd['vd_stock'] != (int)$item['vd_stock']){
					$data_actual = $coma.'Stock ['.$arr_vd['vd_stock'].']';	
					$data_update = $coma.'Stock ['.(int)$item['vd_stock'].']';
				}
				if(!empty($data_update)){
					##-- TXT Log --##
					$msg_log = ' '.str_replace('[DATOS_ACTUALES]',($arr_vd['re_nombre'].' ('.$arr_vd['rc_latitud'].','.$arr_vd['rc_longitud'].'), '.$data_actual),$lang->system->edicion_referencia->__toString());
					$msg_log = str_replace('[DATOS_EDITADOS]',$data_update,$msg_log);
					##-- --##
