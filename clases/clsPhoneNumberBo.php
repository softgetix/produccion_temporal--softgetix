<?php
/** Codificación Bolivia */
require ('clsPhoneNumber.php');


class PhoneNumberBo extends PhoneNumber{
	...
	public function validateGuestNumber(){//--Numero formateado a E164.
		...
		if($this->searchLN($this->getLocalNumber())){	
			...
			$this->setLocalNumberE164('591'.$this->getLocalNumber());
		}
	...
	protected function convertE164ToNational($numberE164){
		$number_aux = null;
		if(substr($numberE164,0,3) == '591'){
			$number_aux = substr($numberE164,3,strlen($numberE164));
		}
		elseif(substr($numberE164,0,4) == '+591'){
	...
	protected function searchLN($number){
		...
		if(substr($number,0,3) == '591' || substr($number,0,4) == '+591'){//--Se quita "591" en caso que tenga
			$number = substr($number,((substr($number,0,3) == '591')?3:4),strlen($number));
		}
