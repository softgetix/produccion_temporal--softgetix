<?php
/** Codificación Ecuador */
require ('clsPhoneNumber.php');

class PhoneNumberEc extends PhoneNumber{
	...
	public function validateGuestNumber(){//--Numero formateado a E164.
		...
		if($this->searchLN($this->getLocalNumber())){	
			...
			$this->setLocalNumberE164('593'.$this->getLocalNumber());
		}
		...
		//--Ini. Validar que tengan 12 y 9 carácteres
	...
	protected function convertE164ToNational($numberE164){
		$number_aux = null;
		if(substr($numberE164,0,3) == '593'){
			$number_aux = substr($numberE164,3,strlen($numberE164));
		}
		elseif(substr($numberE164,0,4) == '+593'){
	...
	protected function searchLN($number){
		...
		if(substr($number,0,3) == '593' || substr($number,0,4) == '+593'){//--Se quita "593" en caso que tenga
			$number = substr($number,((substr($number,0,3) == '593')?3:4),strlen($number));
		}
