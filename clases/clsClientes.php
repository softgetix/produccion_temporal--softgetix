...
class Cliente extends Abm{	
	...
	function obtenerClientes($id=0, $filtro="", $idDistribuidor=0, $perfilAllInOne = false) {
		...
		clientes.cl_id_fletero,clientes.cl_id,clientes.cl_email, clientes.cl_razonSocial, clientes.cl_cuit, clientes.cl_telefono, 
		...
		,clientes.cl_borrado, clientes.cl_email_seguridad, clientes.cl_automatizacion_whatsapp
		";
		if($perfilAllInOne){
			$strSQL.=
	...
	$strSQL.= " AND clientes.cl_id_distribuidor = 74 ";


	}

	if ( $_SESSION['idPerfil'] == 29) {
	$strSQL.= " AND clientes.cl_paquete = 26 ";
