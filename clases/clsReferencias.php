...
class Referencia extends Abm{
    ...
    function obtenerReferenciasPorEmpresa2($idEmpresa = 0, $filtro =  NULL, $idModifica = NULL, $grupo = NULL, $listado = 0) {
		$sql = "exec obtenerReferenciasPorEmpresa {$idEmpresa},'{$filtro}','{$idModifica}','{$grupo}', {$listado},{$_SESSION['idPerfil']}";
	...
	function exiteNumBoca($numboca, $idref = null){
		$strSQL = " SELECT re_id FROM tbl_referencias WITH(NOLOCK) ";
		$strSQL.= " WHERE re_numboca = '".trim($numboca)."' AND re_borrado = 0 
					AND re_us_id IN (SELECT us_id FROM tbl_usuarios WHERE us_cl_id = ".(int)$_SESSION['idEmpresa'].") ";
		if(!empty($idref)){
			$strSQL.= " AND re_id != ".$idref;
		}
	...
	function search($fields = '*', $filter = NULL, $operador_or = NULL){
		$fields = !$fields ? '*' : $fields;
		$sql = " SELECT {$fields} FROM tbl_referencias re WITH(NOLOCK) ";
		$sql.= " INNER JOIN tbl_usuarios us WITH(NOLOCK) ON (us.us_id = re.re_us_id) ";
		$sql.= " INNER JOIN tbl_tipo_referencia tr WITH(NOLOCK) ON (tr.tr_id = re.re_tr_id) ";
		$sql.= " WHERE re_borrado = 0 AND ((";
		
		if($_SESSION['idTipoEmpresa'] != 3){
			$sql.= "us.us_cl_id = ".(int)$_SESSION['idEmpresa'];
		}
		else{
			$sql.= "1=1";
		}
		...
		return $this->objSQL->dbGetAllRows($this->objSQL->dbQuery($sql),3);	
	}
}
