...
class Emailer extends SqlServer{
	...
	function __construct($objSQLServer, $id_smtp = NULL, $logo = NULL) {
		$this->objSQL = $objSQLServer;		
		$this->logo = $logo;
		$this->id_template = 1;
		
		if(empty($id_smtp)){
			if(file_exists('includes/config_'.$_SESSION['DIRCONFIG'].'.php')){
				require('includes/config_'.$_SESSION['DIRCONFIG'].'.php');
				$id_smtp = $arrMail['smtp_envio'];	
			}
		}
		$this->id_smtp = $id_smtp?$id_smtp:1;
	}
	
	function getContenidoHTML($cuerpo_mail){
		if(empty($this->logo)){
			$sql = "SELECT ees_logo FROM tbl_emailer_envios_smtp WITH(NOLOCK) WHERE ees_id = ".(int)$this->id_smtp;
			$query = $this->objSQL->dbQuery($sql);
			$result = $this->objSQL->dbGetRow($query,0,3);
			$this->logo = $result['ees_logo'];
		}
		
		$sql = "SELECT eet_template FROM tbl_emailer_envios_template WITH(NOLOCK) WHERE eet_id = ".(int)$this->id_template;
		$query = $this->objSQL->dbQuery($sql);
		$result = $this->objSQL->dbGetRow($query,0,3);
