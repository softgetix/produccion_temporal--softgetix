...
class Informes extends SqlServer{
	...
	function getInformesAEnviar($filtro){
		$strSQL = " SELECT in_id, CONVERT(TEXT,in_consulta) AS in_consulta, in_subject, in_mensaje, in_adjunto, in_adjunto_name,in_enviar_a_txt, in_enviar_a_us_id, in_enviar_copia_a";
	...
	function generarEnvio($subject, $message, $email, $adjunto = false){
		...
		//-- Enviar Mail --//
		$maqueta = file_get_contents('includes/EnvioMails.html');
		$logo = '<p><span style="'.$styleTXT.'"><a href="http://www.localizar-t.com.ar/" style="text-decoration:none;color:#0A54A8"><span style="text-decoration:none;font:bold 27pt arial;"><span>Localizar-</span><span style="color:#F57C12">T</span></span></a></span></p>';
		$mensaje = '
				<div>
		...
		$replace = array($mensaje,$logo);
		$mensaje = str_replace($search, $replace, $maqueta);
		...
		if($email['cc']){
			...
			$emailerMsg['contenido'] = decode($mensaje);
