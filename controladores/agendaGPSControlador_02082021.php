...
function index($objSQLServer, $seccion, $mensaje="", $filtroCol = false) {
	...
	if($arrViajes){
		...
		if($seccion == 'entregasforza'){
			...
			$objFiltroCol->value($col['estado_check'],'Pendiente','-1');
			$objFiltroCol->value($col['estado_check'],'Rechazado','2');
			$objFiltroCol->value($col['estado_check'],'Confirmado','1');
		}
    ...
    if (!empty($_POST['id_geozonas'])){
		...
		foreach($ref as $ref_id){
			...
			if(isset($_POST['pallets_stock_'.$ref_id])){
				$arrDestinos[] = array('vd_orden' => $i, 'vd_re_id' => $ref_id, 'vd_ini' =>$f_ini, 'vd_fin' =>$f_fin, 'vd_stock' =>intval($_POST['pallets_stock_'.$ref_id]));	
			}
			else{
