...
function index($objSQLServer, $seccion, $mensaje=""){
	...
	$fdesde = isset($_POST['fdesde']) ? date('Y-m-d', strtotime($_POST['fdesde'])) : NULL;
	$fhasta = isset($_POST['fhasta']) ? date('Y-m-d', strtotime($_POST['fhasta'])) : NULL;
	
	require_once 'clases/ADT.php';
	$objADT = new ADT($objSQLServer);

	$tables = $objADT->getContenidoTableroControl(0, $fdesde, $fhasta);
...
function exportar_xls($objSQLServer, $seccion){
	...
	$tables = $objADT->getContenidoTableroControl(1,$fdesde, $fhasta);
	if($tables){
		...
		foreach($tables as $keyTable =>  $table){
			...
			!empty($titleTab) ? $objPHPExcel->getActiveSheet()->setTitle(trim(encode($titleTab))) : NULL;
		}
