...
function index($objSQLServer, $seccion, $mensaje=""){
	...
	$arrArchivos = array();	
	$adj = PATH_ATTACH.'/adt/adtcargafacturas';
	if(file_exists($adj)){
		$handle = opendir($adj);
		sort($handle);
...
function uploadExcel($objSQLServer, $seccion){//--php.ini -->upload_max_filesize=8M
	global $lang;
		...
		switch ((int)$_FILES["archivo"]["error"]) {
			case UPLOAD_ERR_OK:
				...
				if ($extens != 'xls' && $extens != 'xlsx'){
					...
					$resultado = $objADT->Importar_Excel_Facturas($_FILES['archivo']);
					switch ($resultado) {
