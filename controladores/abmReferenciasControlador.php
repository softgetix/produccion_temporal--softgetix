...
function index($objSQLServer, $seccion, $mensaje = "") {
	...
	elseif($action == 'stock'){
		...
		$filterTipo = tienePerfil(19) ? '119,120' : (tienePerfil(27) ? '120' : NULL);		
		$arrEntidades = $objReferencia->obtenerReferenciasPorEmpresa2($_SESSION["idEmpresa"], $txtFiltro, NULL, $filterTipo);
...
function alta($objSQLServer, $seccion, $mensaje = "", $popup = false) {
	...
	if(tienePerfil(array(5,8,9,12,19,27))){
...
function guardarA($objSQLServer, $seccion){
	...
	if(empty($mensaje)){
		...
		if(tienePerfil(array(9,10,11,12,5,6,7,19,29))){
			$campos.= ",re_email";
			$valorCampos.= ",".((isset($_POST["txtEmail"]) && $_POST["txtEmail"]!='')?"''".$_POST["txtEmail"]."''":"null");
		}

		if(tienePerfil(array(9,27))){
			$campos.= ",re_recoleccion_re_id";
			$valorCampos.= ",".((isset($_POST["idrecolecta"]) && $_POST["idrecolecta"]!='')?"''".$_POST["idrecolecta"]."''":"null");
		}

		if(tienePerfil(array(19,27))){
			$campos.= ",re_papel";
			$valorCampos.= ",".(!empty($_POST['re_papel']) ? '1' : '0');
		}

		if(tienePerfil(19)){
			$campos.= ",re_numero_vale";
			$valorCampos.= ",".(!empty($_POST['re_numero_vale']) ? '1' : '0');

			$campos.= ",re_vencimiento";
			$valorCampos.= ",".((isset($_POST['re_vencimiento']) && !empty($_POST['re_vencimiento'])) ? $_POST['re_vencimiento'] : "null");

			$campos.= ",re_Verificado";
			$valorCampos.= ",".(!empty($_POST['re_Verificado']) ? '1' : '0');

			$campos.= ",re_estrella_flexibilidad";
			$valorCampos.= ",".(!empty($_POST['re_estrella_flexibilidad']) ? '1' : '0');

			$campos.= ",re_estrella_calidad";
			$valorCampos.= ",".(!empty($_POST['re_estrella_calidad']) ? '1' : '0');

			$campos.= ",re_estrella_tiempo_turno";
			$valorCampos.= ",".(!empty($_POST['re_estrella_tiempo_turno']) ? '1' : '0');

			$campos.= ",re_estrella_consolidar";
			$valorCampos.= ",".(!empty($_POST['re_estrella_consolidar']) ? '1' : '0');

			//$campos.= ",re_estrella_original";
			//$valorCampos.= ",".(!empty($_POST['re_estrella_original']) ? '1' : '0');
		}
	}
...
function guardarM($objSQLServer, $seccion) {
	...
	if(tienePerfil(27)){
		...
		if(empty($mensaje)){
			if(isset($_POST["txtBoca"]) && $_POST["txtBoca"]!=''){
				if($objReferencia->exiteNumBoca($_POST["txtBoca"], $idReferencia)){
					$mensaje.="* ".$lang->message->error->msj_alta->__toString().'('.$lang->system->num_boca->__toString().')<br/> ';
				}
			}
		}
    ...
    if(empty($mensaje)){
		...
		if(tienePerfil(array(19,27))){
			$set.= ",re_papel = ".(!empty($_POST['re_papel']) ? '1' : '0');
		}
		
		if(tienePerfil(19)){
			$set.= ",re_numero_vale = ".(!empty($_POST['re_numero_vale']) ? '1' : '0');

			$set.= ",re_vencimiento = ".((isset($_POST['re_vencimiento']) && !empty($_POST['re_vencimiento'])) ? $_POST['re_vencimiento'] : "null");

			$set.= ",re_Verificado = ".(!empty($_POST['re_Verificado']) ? '1' : '0');

			$set.= ",re_estrella_flexibilidad = ".(!empty($_POST['re_estrella_flexibilidad']) ? '1' : '0');

			$set.= ",re_estrella_calidad = ".(!empty($_POST['re_estrella_calidad']) ? '1' : '0');

			$set.= ",re_estrella_tiempo_turno = ".(!empty($_POST['re_estrella_tiempo_turno']) ? '1' : '0');

			$set.= ",re_estrella_consolidar = ".(!empty($_POST['re_estrella_consolidar']) ? '1' : '0');

			//$set.= ",re_estrella_original = ".(!empty($_POST['re_estrella_original']) ? '1' : '0');
		}
	}
...
function export_xls($objSQLServer, $seccion){
	if(tienePerfil(27)){
		exportar_xls_stock($objSQLServer, $seccion);
		exit;
	}	
...
function exportar_xls_stock($objSQLServer, $seccion){
	global $lang;
	...
	foreach($arrEntidades as $row){		
		...
		$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('I'.$i, ((int)$row['stock_cliente'] < 0) ? 'Transacción en curso' : (int)$row['stock_cliente']);
		$i++;	

		//--Ini. Solapa 2 Stock
		if((int)$row['stock_cliente'] >= 0){
			...
			if($arrListado){
				foreach($arrListado as $item){
					$objPHPExcel->setActiveSheetIndex(1)
						->setCellValue('A'.($k), encode($item[0]))
						->setCellValue('B'.($k), encode($item[1]))
						->setCellValue('C'.($k), encode($item[2]))
						->setCellValue('D'.($k), encode($item[3]))
						->setCellValue('E'.($k), encode($item[4]))
						->setCellValue('F'.($k), formatearFecha($item[5]))
						->setCellValue('G'.($k), encode($item[6]))
						->setCellValue('H'.($k), encode($item[7]))
						->setCellValue('I'.($k), encode($item[8]))
						->setCellValue('J'.($k), encode($item[9]))
						->setCellValue('K'.($k), encode($item[10]))
						->setCellValue('L'.($k), encode($item[11]));
					$k++;
				}
			}
		}
