...
function popupPostNewConfig($objSQLServer){
	...
	if(empty($tractor) || empty($semi) || empty($configuracion) || empty($carga_bruta) || empty($transportista) || empty($tara)){
		...
		$exec = "EXEC db_ws_new_configuration '{$tractor}', '{$semi}', {$configuracion}, {$carga_bruta}, {$transportista}, {$tara}";
		...
		if($result[0]){
			...
			$status = false;
			$message = 'Los datos no puedieron ser registrados. Por favor verifique los datos ingresados.'.$exec;
		}
	}
