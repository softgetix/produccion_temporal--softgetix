...
function exportar_xls($objSQLServer, $seccion){
	...
	$objPHPExcel->getProperties()
		...
		->setCellValue('A1','TELEFONOS EN CERCANIA')
		->setCellValue('B1','FECHA EN LA QUE SE ROMPIO EL DISTANCIAMIENTO SOCIAL');
	...
	foreach($result as $row){
		$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValueExplicit('A'.$i, encode($row['phone']),PHPExcel_Cell_DataType::TYPE_STRING)
			->setCellValue('B'.$i, formatearFecha($row['contact']));			
		$i++;	
	}}
