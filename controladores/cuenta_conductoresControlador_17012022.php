...
function listado($objSQLServer, $seccion, $mensaje=""){
	...
	if($_GET['action'] == 'trip'){
        popupTrip($objSQLServer, $_GET['co_telefono']);
        exit;
	}
...
function solapaModificar($objSQLServer, $seccion="", $mensaje="", $id=0){
	...
	//-------Ver combo Moviles -------//
	if(tienePerfil(array(5,6,9,10,13,14,19))){
...
function solapaGuardarA($objSQLServer, $seccion){
	...
	//-------Ver combo Moviles -------//
	if(tienePerfil(array(5,6,9,10,13,14,19))){
		if($_POST['cmbMovilAsignado']!=0 || $_POST['cmbMovilAsignado']==0){
			require_once 'clases/clsMoviles.php';
			$objMovil = new Movil($objSQLServer);		
		}
		else{
   ...
   //FIN FRAGMENTO
	if(!$mensaje){
		require_once 'clases/clsConductores.php';
		$objConductor = new Conductor($objSQLServer);
		if($objConductor->insertarRegistro($campos,$valorCampos)){
			//------------//
			if(tienePerfil(array(5,6,9,10,13,14,19)) && $_POST['cmbMovilAsignado']!=0){   		
...
function solapaExportar_xls($objSQLServer, $seccion){
	...
	foreach($arrEntidades as $row){
		$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('F'.$i, encode($row['co_Estado_app']));
...
function popupTrip($objSQLServer, $telefono){ 

	$url = "https://localizar-t.com/trip/index.php?LANG=es_US&IMEI=1&mobilephonenumber={$telefono}&action=tripstatus";
	$strip = file_get_contents($url);
	
	$seccion = 'cuenta_conductores';
    $popup = true;
