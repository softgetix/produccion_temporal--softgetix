...
function alta($objSQLServer, $seccion, $mensaje = "", $datosCargados = array(), $popup = false, $arrDestinos = array()){
	...
	if(tienePerfil(array(19,29)) && $seccion == 'retirosforza'){
...
function controlarCampos($esAlta = 0) {
	...
	elseif(tienePerfil(array(19,29)) && $_REQUEST['hidSeccion'] == 'retirosforza'){
		$campos[] = 'vi_dador';
		$valorCampos[] = $_POST['dador']?$_POST['dador']:NULL;
	}
	/*global $objSQLServer;
    ...
    if (!empty($_POST['id_geozonas'])){
		...
		elseif($_REQUEST['hidSeccion'] == 'retirosforza'){//--Se agrega punto de retorno a viajes de recupero
			$auxDestino = array('vd_orden' => $i, 'vd_re_id' => 90581);
			if(isset($_POST['fecha_'.$ref_id])){//--Se le grega la fecha del ultimo punto cargado.
				$auxDestino['vd_ini'] = date('Y-m-d H:i',strtotime($_POST['fecha_'.$ref_id].' '.$_POST['hora_'.$ref_id].':'.$_POST['min_'.$ref_id]));
			}
			else{
				$auxDestino['vd_ini'] = date('Y-m-d H:i');
			}
			$arrDestinos[] = $auxDestino;
		}
	}
