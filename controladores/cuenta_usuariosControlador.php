...
function solapaGuardarM($objSQLServer, $seccion){
	...
	$resp = validarCampos($_POST, $idUsuario);
	$mensaje = $resp['mensaje'];
	$coma = '';
...
function validarCampos($post, $id){
	global $lang;
	global $objSQLServer;
	

	//--Valida que el nro de teléfono no se repita
	if(!empty($post['txtTelefono'])){
		$query = "SELECT COUNT(*) as cant FROM tbl_usuarios WHERE us_cl_id = ".(int)$post['cmbCliente']." AND us_telefono = '".trim($post['txtTelefono'])."'";
		if($id){ //--Es modificación
			$query.= " AND us_id != ".(int)$id;
		}

		$reg = $objSQLServer->dbGetRow($objSQLServer->dbQuery($query),0,3);
		if($reg['cant'] > 0){
			$mensaje.="* Ya se encuentra registrado un usuario con el mismo ".$lang->system->telefono->__toString()."<br/>";
							...
							if(!empty($post[$idCampo])){
								...
								switch($_SESSION['idPais']){
									case 1:
										...
										$objPhone = new PhoneNumberAr($post[$idCampo], $post[$idCampo]);
									...
									case 4:
										...
										$objPhone = new PhoneNumberUy($post[$idCampo], $post[$idCampo]);
										$validPhone = $objPhone->validateGuestNumber();
										if(empty($validPhone['guest_number_e164'])){
