...
function index($objSQLServer, $seccion,$msg=''){
	...
	$query.= ', '.(!empty($_POST['origin_id']) ? $_POST['origin_id'] : '0');
	$query.= ', '.(!empty($_POST['route_id']) ? $_POST['route_id'] : '0');
	$query.= ', '.(!empty($_POST['s_trip_code']) ? "'".$_POST['s_trip_code']."'" : "''");
	$query.= ', '.(!empty($_POST['fecha']) ? "'".date('m-d-Y',strtotime($_POST['fecha']))."'" : "''");
