...
$arr_min = array('00','10','20','30','40','50','59');
...
function alta($objSQLServer, $seccion, $mensaje = NULL,$datosCargados = NULL){
	...
	//--Ini. Fix para no habilitar al usuario logueado a dejar sin usuario a notificar
	$noBorrarUsuario = array();
	foreach($arrUsuarios as $k => $us){
		if($us['us_id'] == $_SESSION['idUsuario']){
			$noBorrarUsuario = $us;
			unset($arrUsuarios[$k]);
		}
	}
	//--Fin. Fix para no habilitar al usuario logueado a dejar sin usuario a notificar
...
function modificar($objSQLServer, $seccion, $mensaje = NULL, $datosCargados = NULL){
	...
	$noBorrarUsuario = array();
	foreach($arrUsuarios as $k => $us){
		if($us['us_id'] == $valorActivo['usuarios'][0]){
			$noBorrarUsuario = $us;
			unset($arrUsuarios[$k]);
		}
	}
