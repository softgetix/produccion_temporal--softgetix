...
function index($objSQLServer, $seccion, $mensaje="", $filtroCol = false){
	...
	if($arrViajes){
		...
		//--INI. Graficos y Estadisticas
		if($_SESSION['idEmpresa'] == 4835 || $_SESSION['idAgente'] == 4835){
			if(!tienePerfil(array(8,12))){
				$grafic = getGraficosArauco();		
			}
		}
		else{
			$grafic = getGraficos($arrViajes);		
		}
		//--FIN. Graficos y Estadisticas		
	}
...
function verDetalle($objSQLServer, $seccion, $mensaje = NULL, $idViaje = false){
	...
	$seguridad = $objViaje->getSeguridad();
...
function bajaViaje($objSQLServer, $seccion = "", $idViaje = NULL){
	global $lang;
...
function getGraficos($arrViajes){
	global $lang;
	##-- ini.GRAFICOS --##
	$title1 = 'Distribución total de viajes';
	$title2 = 'Distribución por transportista';
	$title3 = $lang->system->title_tareas_grafico_3->__toString();
	$title4 = $lang->system->title_tareas_grafico_4->__toString();
	$title5 = 'Confirmación de entregas'; 
	...
	return array(
		'title1' => isset($title1) ? $title1 : NULL
		,'data1' => isset($data1) ? $data1 : NULL
		,'title2' => isset($title2) ? $title2 : NULL
		,'data2' => isset($data2) ? $data2 : NULL
		,'title3' => isset($title3) ? $title3 : NULL
		,'data3' => isset($data3) ? $data3 : NULL
		,'title4' => isset($title4) ? $title4 : NULL
		,'data4' => isset($data4) ? $data4 : NULL
		,'title5' => isset($title5) ? $title5 : NULL
		,'data5' => isset($data5) ? $data5 : NULL
		,'stats' => isset($stats) ? $stats : NULL
	);
}

function getGraficosArauco(){
	global $lang;
	global $objSQLServer;
	
	$hoy = date('Y-m-d');
	$ultimomes = date('Y-m-d', strtotime($hoy.'- 30 days')); 

	//--Ini. Grafico 1
	$title1 = $lang->system->title_tareas_grafico_1->__toString();
	$query = "EXEC db_kpi_Arauco_asignaciones {$_SESSION['idUsuario']}, '{$hoy} 00:00', '{$hoy} 23:59'";
	$result = $objSQLServer->dbGetAllRows($objSQLServer->dbQuery($query));
	if($result){
		$data1 = "['Descripción', 'Valor']";
		foreach($result as $item){
			$data1.= ",['".encode($item['descripcion'])."',".(int)$item['value']."]";
		}
	}
	//--Fin. Grafico 1

	//--Ini. Grafico 2
	$title2 = $lang->system->title_tareas_grafico_2->__toString();
	$query = "EXEC db_kpi_Arauco_distribucion {$_SESSION['idUsuario']}, '{$hoy} 00:00', '{$hoy} 23:59'";
	$result = $objSQLServer->dbGetAllRows($objSQLServer->dbQuery($query));
	if($result){
		$aux = array();
		$aux_title = array();
		...
		foreach($result as $item){
			if(!isset($aux_title[$item['tipo']])){
				$aux_title[$item['tipo']] = $item['descripcion'];
			}
			
			$aux[$item['Transporte']][$item['tipo']] = (int)$item['value'];
		}
			
		$data2 = "['Transporte'";
		foreach($aux_title as $desc){
			$data2.= ", '{$desc}'";
		}
		$data2.= "]";
			
		
		foreach($aux as $K => $item){
			$data2.= ",['".encode($K)."'";
			foreach($aux_title as $kt  => $itemt){
				$data2.= ", {$item[$kt]}";
			}
			$data2.= "]";
		}
	...
	//--Ini. Grafico 3
	$title4 = $lang->system->title_tareas_grafico_4->__toString();
	$query = "EXEC db_kpi_Arauco_distribucion {$_SESSION['idUsuario']}, '{$ultimomes} 00:00', '{$hoy} 23:59'";
	$result = $objSQLServer->dbGetAllRows($objSQLServer->dbQuery($query));
	if($result){
		$aux = array();
		$aux_title = array();
