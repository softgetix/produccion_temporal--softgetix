...
function index($objSQLServer, $seccion, $mensaje="", $filtroCol = false) {
	...
	$diffFechas = ($_POST['fdesde'] == getFechaServer('Y-m-d'))?0:(abs((strtotime($_POST['fdesde'])-strtotime($_POST['fhasta']))/86400));
...
function filtroFecha($objSQLServer, $seccion){
	...
	switch($tipo){
		...
		break;*/
		case 'ultimo_mes':
			$_POST['fdesde'] = date('Y-m-d',strtotime('-31 day',strtotime($hoy)));
			$_POST['fhasta'] = date('Y-m-d',strtotime('-1 day',strtotime($hoy)));	
		break;
		/*case 'prox_dias':
	...
	setcookie('filtro_desde', $_POST['fdesde'], time()+180); /* expira en 3 min */
	if(empty($_POST['fhasta'])){
...
function controlarCampos($esAlta = 0) {
	...
	/*$msjError = checkCombo((int)$_POST['transportista'], $lang->system->flota, 1, 0);
	if ($msjError){
        $mensaje.="* " . $msjError . "<br/> ";}	
	*/
    ...
    if (!empty($_POST['id_geozonas'])){
		...
		foreach($ref as $ref_id){
			$f_ini = date('Y-m-d H:i',strtotime($_POST['fecha_'.$ref_id].' '.$_POST['hora_'.$ref_id].':'.$_POST['min_'.$ref_id]));
			
			$f_fin = NULL;
			if(isset($_POST['f_egreso_'.$ref_id])){
				$f_ini = date('Y-m-d H:i',strtotime($_POST['f_egreso_'.$ref_id])); 
			}
			
			$stock = isset($_POST['pallets_stock_'.$ref_id]) ? intval($_POST['pallets_stock_'.$ref_id]) : null;
			
			$arrDestinos[] = array('vd_orden' => $i, 'vd_re_id' => $ref_id, 'vd_ini' =>$f_ini, 'vd_fin' =>$f_fin, 'vd_stock' =>$stock);	
			$i++;
		}
	}
	else{
