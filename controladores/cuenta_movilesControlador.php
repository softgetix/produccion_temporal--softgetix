...
function listado($objSQLServer, $seccion, $mensaje = "") {
	...
	elseif($_GET['action'] == 'reactivar_movil'){
        popupReactivarMovil($objSQLServer, $seccion);
        exit;
    }
...
function popupReactivarMovil($objSQLServer, $seccion){
	$idusuario = $_SESSION['idUsuario'];
    $idmovil = $_GET['idmovil'];
	$matricula = $_GET['matricula'];

    $strQuery = "EXEC dbo.reactivar_patente {$idmovil}, '{$matricula}'";
    $results = $objSQLServer->dbGetAllRows($objSQLServer->dbQuery($strQuery));

	$message = 'El móvil se activo con éxito';
