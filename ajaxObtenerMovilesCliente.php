...
if($idCliente>-1){
	...
	$arrMoviles = $objMovil->obtenerMovilesClientesCombo($idUser, $idCliente, $idDistribuidor);
	$noptions = 0;
	for($i=0;$i < count($arrMoviles) && $arrMoviles;$i++){
		$return .= "cmb.options[" . $noptions . "]=new Option('" .(encode($arrMoviles[$i]["dato"])). "','" .$arrMoviles[$i]["id"]. "');";
		$noptions++;
	}
