...
class WatiApi extends CI_Controller {
	...
	public function WsCall($action, $cred=0){
		...
		if($cred == 1) $wsUrl = 'https://www.localizar-t.com/gateway/wswati.php';	
		...
		switch ($action) {
			  ...
			  case 'NewMessageStatus':
			  	...
			  	$phone = isset($_POST['wp_number']) ? $_POST['wp_number'] : '';
	...
	public function getMessages()
	{
      ...
      if (!empty($response_array->info) && $response_array->info == 'Invalid Conversation') {
    	...
    	$response = array('html'=>$html,'new_message_status' => json_decode($new_message_status));
