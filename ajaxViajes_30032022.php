	...
	switch ($_POST['accion']){
		...
		case 'get-geozona-ruteo':
			...
			if(!empty($_POST['geozona'])){
				...
				if(tienePerfil(array(19,27)) && $_SESSION['seccion'] == 'retirosforza'){ //--retiros
					$datos['idgrupo'] = 119;
	
					$query = "SELECT re_id FROM tbl_referencias WHERE re_rg_id = 120 AND re_borrado = 0 ";
					$rows = $objSQLServer->dbGetAllRows($objSQLServer->dbQuery($query), 0, 3);
					if($rows){
						$coma = '';
						foreach($rows as $item){
							$datos['zona_compartida'].= $coma.$item['re_id'];		
							$coma = ',';
						}
					}
				}
				elseif(tienePerfil(array(19,27))){ //--entregas
					$datos['idgrupo'] = 120;
					//$datos['zona_compartida'] = '63442,63441,63440,63439,63438,63437,63436,63435,63434';
				}
		...
		case 'autocomplete-geozonas':
			...
			###
			/*$zonas['geozonas'][0]['etiqueta'] = 'KC SAN LUIS 2';
			$zonas['geozonas'][0]['re_nombre'] = 'KC SAN LUIS 2';
			
			$zonas['geozonas'][1]['etiqueta'] = 'KC CLAP';
			$zonas['geozonas'][1]['re_nombre'] = 'KC CLAP';
			/**/
			###
