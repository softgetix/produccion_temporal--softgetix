...
#buscador-movil, #buscador-conductor{left:5px;}
    	...
    	<tr>
        	<td style="padding:10px;">
            	<?php if($tieneMovilAsignado){?>
                	<?php if($idViaje){ ?>									
					    <a href="javascript:;" class="button colorin" style="width:173px;"
                            onclick="javascript:enviar('guardarM')"><?=$lang->botonera->guardar?></a>
                        ...
                        <?php }?>
                    <?php }else{ ?>
						<a href="javascript:enviar('<?=$idViaje?'guardarM':'guardarA'?>')" id="btn-guardar" class="button colorin gum" style="width:173px;">
							<?=$lang->botonera->guardar?></a>												
					<?php }
                 }
				else{?>
                ...
                <fieldset class="gum_top2" id="fieldValidarCodigo">
                	<label class="placeholder txt_ie none" style="text-align:left"><?=$lang->system->codigo_viaje?></label>
                    <?php if(tienePerfil(5)){?> 
                    <input type="text" name="cod_viaje" id="cod_viaje" value="<?=$arrViaje['vi_codigo']?>" class="no_margin float_l mitad_largo" placeholder="<?=$lang->system->codigo_viaje?>" onkeyup="javascript:onChanges(null,null)" onblur="javascript:validarCodigoViaje(this.id)"><span class="obligatorio">*</span>
                    <?php }else{ ?>
                        <input type="text" value="<?=$arrViaje['vi_codigo']?>" class="no_margin float_l mitad_largo" readonly>
                    <?php }?>
                </fieldset>

                <fieldset class="gum_top2" id="fieldValidarCodigo">
                	<label class="placeholder txt_ie none" style="text-align:left">Contenedor</label>
                    <input type="text" name="vi_contenedor" value="<?=$arrViaje['vi_contenedor']?>" class="no_margin float_l mitad_largo" placeholder="Contenedor" onkeyup="javascript:onChanges(null,null)">
                </fieldset>
                
                <fieldset class="gum_top2">
                    <?php if(tienePerfil(8)){?>
                        <?php foreach($tipo_viaje as $item){
                            if($item['vt_id'] == $arrViaje['vt_id']){?>
                                <input type="text" value="<?=encode($item['vt_nombre'])?>" class="no_margin float_l mitad_largo" readonly>
                            <?php }?>
                        <?php }?>
                    <?php } 
                    else{?>
                    <select name="tipo_viaje" id="tipo_viaje" class="no_margin float_l mitad_largo" onchange="javascript:onChanges(null,null)">
                        <option value="" class="placeholder"><?=$lang->system->tipo_viaje?></option>
                        <?php foreach($tipo_viaje as $item){?>
                ...
                <fieldset class="gum_top2 none">
                	<select name="transportista" id="transportista" class="no_margin float_l mitad_largo" onchange="javascript:onChanges(this, 'comboTransportista')">
                        <option value="" class="placeholder"><?=$lang->system->transportista?></option>
                    </select>
                    <span id="tr-reload"></span>
                    <span class="obligatorio">*</span>
                    <input type="hidden" id="temp_transportista" name="temp_transportista" value="<?=$arrViaje['vi_transportista']?>" />
                </fieldset>
                ...
                <div id="DatosCamposOpcionales" class="clear" style="display:none;">                	
                    <fieldset class="gum_top2">
                        ...
                        <textarea name="observaciones" id="observaciones" class="no_margin" rows="5" style="width:160px;" onkeyup="javascript:onChanges(null,null)" <?=tienePerfil(8)?'readonly':''?>><?=trim($arrViaje['vi_observaciones'])?></textarea>
                    </fieldset>
					<?php if(tienePerfil(5)){?>
                    <fieldset class="gum_top2">     
                        <span class="float_l" style="line-height:22px;"><?=$lang->system->facturado?></span>
                        <input type="checkbox" name="vi_facturado" value="1"  style="width:20px;margin-bottom:0px" class="float_r" <?php if($arrViaje['vi_facturado']){?>checked="checked"<?php }?> onchange="javascript:onChanges(null,null)"> 
                    </fieldset>
            ...
            <td width="100%">
            	<div class="solapas gum clear" style="overflow:hidden;"><!-- height:92%;-->
                    ...
                    <?php if($historial[0] && tienePerfil(5)){?>
