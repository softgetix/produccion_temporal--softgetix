            ...
            foreach($arrEntidades as $i => $item){
                ...
                <tr class="<?=$class?> <?=((count($arrEntidades) - 1)==$i)?'tr-last':''?> <?=$item['cl_borrado']?'row_inactivo':''?>">
                   ...
                   <?php }
                    	...
                    	<td>
                        <?php if(tienePerfil(array(5,9,13,19,27,32,33,36)) && !$item['cl_borrado']){?>
					...
					<td class="no_padding td-last">
                    	<center>
							...
							<?php } else{
								if(tienePerfil(5) && !empty($item['cl_email'])){
									$ide = 're-email-'.$i; ?>									
									<a href="javascript:CreateAccount(<?=$item['cl_id']?>, '<?=$ide?>'); SendMailResetPass('<?=$item['cl_email']?>','<?=$_SESSION['DIRCONFIG']?>','<?=$ide?>', true);" title="<?=$lang->system->title_cambio_clave?>" id="<?=$ide?>"><img src="imagenes/send_mail.png" /></a>
								<?php }
								if(tienePerfil(array(5,9,13,19,27))){?>
