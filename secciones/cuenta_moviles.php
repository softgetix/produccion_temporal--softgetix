...
switch($operacion){
	case 'modificar':?>
		...
		<?php if(tienePerfil(array(19,37))){?>
        <fieldset>
        ...
        <?php if($arrEntidades){
            foreach($arrEntidades as $i => $item){
                ...
                <tr class="<?=$class?> <?=((count($arrEntidades) - 1)==$i)?'tr-last':''?>  <?=$item['mo_borrado']?'row_inactivo':''?>">
 					<td>
						<?php if(tienePerfil(array(5,9,13,19,37)) && !$item['mo_borrado']){?>
                        ...
                        <?php }
                        elseif(tienePerfil(array(19,37))) {?>
   					        <input type="hidden" name="chkId[]" id="chk_<?=$item['mo_id']?>" value="<?=$item['mo_id']?>"/>
                            <a href="javascript:enviarModificacion('modificar',<?=$item['mo_id']?>)"><?=$item['mo_matricula']?></a>
						<?php }					
                    ...
                    <?php }?>
                    <td><center><?=$arrEntidades[$i]['sh_fechaRecepcion']?></center></td>
                    <td class="no_padding td-last">
                    	<center>
                            <?php if($item['mo_borrado'] && tienePerfil(array(19,37))){?>
                                <a title="Enviar Mensaje" class="reactivar_movil" href="javascript:;" attrIdMovil="<?=$item['mo_id']?>"  attrMoMatricula="<?=$item['mo_matricula']?>" style="margin-top:4px; text-align: center;">
                                    <span>Reactivar móvil</span>
                                </a>
                            <?php }elseif($item['mo_borrado'] ){?>
                            	<span><?=$lang->system->movil_inactivo?></span>	
                            <?php }elseif(tienePerfil(array(5,9,13,19,37))){?>
