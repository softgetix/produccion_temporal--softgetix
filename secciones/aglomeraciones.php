...
$(document).ready(function(){
   timeout = setInterval(function () {
      if (google.visualization != undefined) {
        var $titulo = 'Aglomeraciones por hora'; 
        var $ide = 'grafico1';
        var $cant = <?=count($data1)?>;
        var $data1 = google.visualization.arrayToDataTable([<?=$data1?>]);
        drawChartBarras($data1,$titulo,$ide,["#5CB85C"]);
                ...
                <table width="100%" height="100%">
                    <thead>
                        <tr>
                            ...
                            <td><span class="campo1"><center>Tel&eacute;fonos en cercan&iacute;a</center></span></td>
                            <td class="td-last"><span class="campo1"><center>Fecha en la que se rompi&oacute; el distanciamiento social</center></span></td>
                        ...
                        <?php if($result){
                            foreach($result as $i => $item) {
                                ...
                                <tr class="<?=$class?> <?=((count($result)-1) == $i)?'tr-last':''?>">
                                    ...
                                    <td class="td-last"><center><?=formatearFecha($item['contact'])?></center></td>
