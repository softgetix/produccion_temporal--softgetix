...
<div id="main" class="sinColIzq">
    ...
    <!--<div class="mainBoxLICabecera">-->
        <form name="frm_<?=$seccion ?>" id="frm_<?=$seccion?>" action="?c=<?=$seccion?>" method="post" style="height:100%;" enctype="multipart/form-data">
            ...
            <a class="izquierda float_l <?=($solapa=='adt_terminos_y_condiciones')?'active':''?>" href="<?=$_SERVER['REDIRECT_URL'].'?c='.$seccion.'&solapa=adt_terminos_y_condiciones'?>">T y C</a>
	        ...
	        <a class="izquierda float_l <?=($solapa=='adt_panel')?'active':''?>" href="<?=$_SERVER['REDIRECT_URL'].'?c='.$seccion.'&solapa=adt_panel'?>">Panel inteligente</a>
	        <a class="izquierda float_l <?=($solapa=='adt_contrato')?'active':''?>" href="<?=$_SERVER['REDIRECT_URL'].'?c='.$seccion.'&solapa=adt_contrato'?>">Contrato electr&oacute;nico</a>
            <div style="height:100%" class="contenido flaps clear"> 
                    ...
                    <?php if($arrListado){
                        foreach($arrListado as $i => $item){?>
                            <tr class="<?=((count($arrListado) - 1)==$i)?'tr-last':''?>">
                                ...
                                <center>
                                    <br>
                                    <?php if($_GET['con_pdf'] == 1){
                                        $extension = pathinfo($item['ac_url'], PATHINFO_EXTENSION);
                                        if(strtolower($extension) == 'pdf'){?>
                                            <embed src="<?=$item['ac_url']?>" type="application/pdf" width="100%" height="800px" />
                                        <? }
                                        else{?>
                                            <img src="<?=$item['ac_url']?>">  
                                        <?php }?>
                ...
                <thead>
                    <tr>
                        <td><center><span class="campo1">Documentos disponibles</span></center></td>
                ...
                <?php if($arrListado){
                    foreach($arrListado as $i => $item){
                        ...
                        <tr class="<?=$class?> <?=((count($arrListado) - 1)==$i)?'tr-last':''?>">
                            ...
                            <!--<td><a href="<? //=$item['ac_url']?>" target="_blank"><? //=$item['ac_url']?></a></td>-->
                        </tr>
                    <?php } 
