...
<table width="100%" height="100%">
    <thead>
        <tr>
            ...
            <td class="td-last"><center><span class="campo1">Estado de la App (al recibir los datos)</span></center></td>
        </tr>
    </thead>
    <tbody>
    <?php 
    ...
    if($arrEntidades){
        foreach($arrEntidades as $i => $item){
            ...
            <tr class="<?=$class?> <?=((count($arrEntidades) - 1)==$i)?'tr-last':''?>">
                ...
                <td class="td-last"><center><?=isset($auxarr[$item['app_status']]) ? $auxarr[$item['app_status']] : $auxarr[2]?></center></td>
            </tr>
        <?php } 
        }
        else{?>
