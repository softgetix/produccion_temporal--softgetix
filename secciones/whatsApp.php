...
?>
  ...
  .app {  position: relative;  overflow: hidden;  top: 19px;  height: calc(100% - 15px);  margin: auto;  padding: 0;  box-shadow: 0 1px 1px 0 rgba(0, 0, 0, .06), 0 2px 5px 0 rgba(0, 0, 0, .2);}
  ...
  .reply-recording { padding: 5px !important;text-align: right;}
  .reply-recording i {  text-align: right;  padding: 5px !important;  color: #93918f;  cursor: pointer;}
  ...
  .toast-top-center { 
   ...
   /* Clearable text inputs */
    ...
    #stopButton,#removeRecord {color: #ff0016;}
    #playButton {color: #32aa60;}
   .button-animate {animation: pulse 1s infinite;}
   ...
   /* Hide the images by default */
    ...
    .mySlides {padding-top:125px; }
     #image_slider {background-color: #e8edef;}
     .mySlides img{width: 65%;object-fit: contain;margin: 0 auto;max-height: 180px}
     #preview_send {background: #00a986;padding: 12px;color: #fff;cursor: pointer;}
     #preview_close {color: #fff;padding: 4px;background: #b80e0e;cursor: pointer;}
     #image_slider .no-preview {border: 1px solid #00aa88;text-align: center;font-weight: 700;    display: flex;align-items: center;justify-content: center;word-break: break-all;}
     .row.image-row {display: flex;overflow-x: auto;}
    </style>


<div id="wp-container" class="container app" style="width:1340px;">
 ...
 -->
  ...
  <div class="col-sm-8 conversation" id="chat_screen" style="display: none;">
   <div class="col-md-12">
    <div class="container" style="position:fixed;z-index: 99;padding-left: 0;">
     <div class="" id="preview_containter" style="display: none;width:75.5%; height:495px;background: #e8edef;border: 1px solid #00aa88; position:relative;transition: visibility .5s, opacity 0.5s linear;box-shadow: 0 0 10px #00aa88;padding: 10px;">
      <span style="position: absolute;" id="preview_close"><i class="fa fa-times fa-2x"></i></span>

      <span style="position: absolute;top: 89%;left: 89%;"><i class="fa fa-send fa-2x" id="preview_send" aria-hidden="true"></i></span>

        <div style="width:100%;height: 100%; background-color:#e8edef;">
                    <!-- Slider start -->
               <div class="slide-dev" > 
                  <div id="image_slider" >
                    <div class="mySlides no-pre-slide">
                   ...
                   <!--  <a class="prev" onclick="plusSlides(-1)">❮</a>
                    ...
                    <div class="row image-row" style="margin-left:0;">
                    </div>
                  </div>
               </div>   
                <!-- End Slider start -->
      ...
      <div class="row reply" id="user_reply">
            ...
            <input type="hidden" id="sendRecording" >
            <input type="hidden" id="selectContactNumber">
            <input type="hidden" id="selectContactDetailId">
            <div class="col-sm-8 col-xs-8 reply-main clearable">
             ...
             <div class="col-sm-2 col-xs-2 reply-recording">
              <img src="<?php echo base_url().'assets/loader.gif' ?>" class="loader recording-loader" style="display: none;position: relative;width: 40px;">

              <i class="fa fa-microphone fa-2x disabled" aria-hidden="true" id="recordButton"></i>
              <i class="fa fa-trash fa-2x" id="removeRecord" style="display:none;" ></i>
              <i class="fa fa-pause fa-2x button-animate" id="stopButton" style="display:none;"></i>
              <i class="fa fa-play fa-2x" id="playButton" style="display:none;"></i>
...
<div id="dummyModal" class="modal modal fade" style="height: fit-content;margin-top: 64px;">
  <div class="modal-dialog" style="margin-left: 947px;margin-right: 0px; width: 400px;">
  ...
  $(document).ready(function(){
         ...
         getContacts();
                          ...
                          var contactlist = result.contactlist;
                           $.each(contactlist,function(index,contactlist){
                            ...
                            if(template == 1){
                              ...
                              $('#comment').val(contactlist.textshown).addClass('disabled').prop('readonly', true);
                              $('#reply-send-btn,#attach-file,#recordButton').addClass('disabled');
                               ...
                               $('#reply-send-btn,#attach-file,#recordButton').removeClass('disabled');
                            }
          ...
          $(document).on("click",'#reply-send-btn',function(){
            if(!$(this).hasClass('disabled')){
              ...
              if($('#sendRecording').val() == 1){ 
                  sendRecording($('#selectContactNumber').val(),fd);
              }else{
                 if($('#sendFiles').val() != 1){
                  sendMessage();
         ...
         $(document).on('click','#back',function(){
          ...
          $(document).on('click','#comment,#reply-send-btn',function(){
            if(template == 1 && $('#sendRecording').val() != 1){
              sendMessage();
            }
          });
          ...
          window.addEventListener('paste', e => {
            if($("#comment").is(":focus") && !$('#attach-file').hasClass('disabled')){
          ...
          function previewFiles(){
            ...
            $('#preview_containter').show();
            $('.slide-dev .mySlides1').remove();
            $('#image_slider .column1').remove();
            var j = 1;
             for (i = 0; i < filesAmount; i++) {
              ...
              console.log(fileInput.files[i].type);
                  ...
                  reader['num'] = parseInt(j+1);
                  j++;
                  reader.onload = function(event) {

                    $('#image_slider .image-row').append('<div class="column1"><div class="demo cursor"><img src="'+event.target.result+'" style="width:150px;height: 100px;object-fit: contain;" onclick="currentSlide('+event.target.num+')" ></div></div>');
                    $('#image_slider').before('<div class="mySlides mySlides1"><img src="'+event.target.result+'" ></div>');
                    showSlides(1);
                  }
                  reader.readAsDataURL(fileInput.files[i]);

                }else{
                   $('#image_slider .image-row').prepend('<div class="column1 no-preview"><div class="demo cursor"><span onclick="noPreview()">'+fileInput.files[i].name+'</span></div></div>');
                  if(j == 1){
                   $('.no-pre-slide').css('display','flex');
                  }
                }  
              }
          }
         ...
         $('#removeRecord').click(function(){
          $('#removeRecord,#playButton').hide();
          $('#recordButton').show();
          $('#sendRecording').val('');
        });
        $('#preview_close').click(function(){
          $('#preview_containter').hide();
          $('.clearable__clear').click();
        });
        $('#preview_send').click(function(){
          $('#reply-send-btn').click();
          $('#preview_containter').hide();
          sendFiles($('#selectContactNumber').val());
        });         
   });
 ...
 function sendMessageWithNumber(contact_number,user_name,send,template,templateName){
   ...
   // pauseButton.addEventListener("click", pauseRecording);
    ...
    $('#playButton').click(function(){
     ...
     function sendRecording(contact_number, formData){
             if(contact_number){      
              formData.append('us_id', us_id);
              formData.append('wp_number', contact_number);
              $('.recording-loader').show();
              $('#sendRecording').val('');
              $('#removeRecord,#playButton').hide();
              $('#recordButton').show();
              $.ajax({
                       url : '<?php echo base_url(); ?>WatiApi/sendRecording',
                       type : 'POST',
                       data : formData,
                       cache: false,
                       contentType: false,
