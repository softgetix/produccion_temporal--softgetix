...
switch($operacion){
	...
	case 'modificar': ?>
		...
		<!---------Ver combo Moviles ------->
		<?php if(tienePerfil(array(9,10,13,19))){ ?>
		<fieldset>
        ...
        <thead>
            <tr>
             ...
             <!--    <td><center><span class="campo1">Verificado</span></center></td>  -->
                <td class="td-last"><center><span class="campo1"><?=tienePerfil(8) ? '' : $lang->botonera->eliminar?></span></center></td>
            </tr>
        </thead>
        <tbody>
        <?php if($arrEntidades){
                	...
                	<td>
                        <?php if(!tienePerfil(array(7,11))){?>
                            ...
                            <a href="javascript: enviarModificacion('modificar',<?=$item['co_id']?>)">
							 	<?=($item['co_nombre'].' '.$item['co_apellido']) . (!empty($item['co_dni']) ? ' (DNI:'.$item['co_dni'].')' : '')?>
							</a>
                        <?php }
						else{ 
							echo ($item['co_nombre'].' '.$item['co_apellido']) . (!empty($item['co_dni']) ? ' (DNI:'.$item['co_dni'].')' : '');
						}?>
					</td>
