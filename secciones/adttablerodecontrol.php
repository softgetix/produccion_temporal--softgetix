 ...
 <div id="main" class="sinColIzq">
    ...
    <form name="frm_<?=$seccion?>" id="frm_<?=$seccion?>" action="?c=<?=$seccion?>" method="post">
        ...
        <?php if(!ES_MOBILE){?>
            ...
            <input type="text"  class="date fdesde " name="fdesde" placeholder="Fecha Inicio" autocomplete="off" value=<?=$_POST['fdesde']?> >
            <input type="text" class="date fhasta " name="fhasta" placeholder="Fecha Fin" autocomplete="off" value=<?=$_POST['fhasta']?>>
           
            <?php } ?>
	   <a id="botonGuardar" class="button_xls " onclick="javascript:enviar('exportar_xls');" style="margin-top:6px;" href="javascript:;">Generar informe</a>
...
<script src="js/wpNotification.js"></script>
