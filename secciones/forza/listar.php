...
if($type == 'entregas'){?>
	<fieldset class="grupo" style="margin-bottom:10px;">
		<?php if($permisoCarga){?>
			<a href="javascript:;" onclick="enviar('alta')" class="button colorin" style="padding:8px 20px;">Nuevo Vale</a>
		<?php }
		elseif($type == 'entregas'){?>
			<a href="https://wa.me/5491171243942" target="_blank" class="button colorin" style="padding:8px 20px;">Nuevo Vale</a>
		<?php }?>
	</fieldset>
	<fieldset class="grupo" style=" margin:10px 0;">
<?php } else{
	$auxstyle = '';
	if(($_SESSION['cargamanualok'] == 1 && tienePerfil(27)) || $permisoCarga) {?>
	<fieldset class="grupo" style="margin-bottom:10px;">
		
		<br />
		<center><a href="javascript:;" onclick="enviar('alta');" class="button colorin" style="margin-top:5px; padding:8px 20px;">Nuevo Recupero</a></center>
		<br />
	</fieldset>
	<?php $auxstyle = 'margin:10px 0;'; } ?>
	<fieldset class="grupo" style="<?=$auxstyle?>">
...
<table width="100%" height="100%">
	<tbody>
		...
		<?php if($type == 'entregas'){?>
		    ...
		    <a class="float_l margin_l button <?=($active_btn == 1) ? 'colorin':''?>" style="margin-top:5px;" onclick="javascript:enviar('filtroFecha','ultimo_mes');" href="javascript:;">Verificado</a>
                    <a class="float_l margin_l button <?=($active_btn == 2) ? 'colorin':''?>" style="margin-top:5px;" onclick="javascript:enviar('filtroFecha','hoy');" href="javascript:;">Pendiente de Verificaci&oacute;n</a>
                 
		<?php } else { ?>
 			<a class="float_l margin_l button <?=($active_btn == 1) ? 'colorin':''?>" style="margin-top:5px;" onclick="javascript:enviar('filtroFecha','ultimo_mes');" href="javascript:;">Realizado</a>
                    <a class="float_l margin_l button <?=($active_btn == 2) ? 'colorin':''?>" style="margin-top:5px;" onclick="javascript:enviar('filtroFecha','hoy');" href="javascript:;">En curso</a>
                    ...
                    <table width="100%" height="100%">
						...
						foreach($arrViajes as $k => $row){
							...
							<tr class="<?=$fila?> <?=(!$verContenido)?'destinos_'.$row['vi_id'].' row_viajes':''?> <?=($row['vi_finalizado'])?'viaje-finalizado':''?>" <?php if(!$verContenido){?>style="display:none" <?php }?>>
								...
								<? 
								if ($type != 'entregas') { 
									$aux = array('0' => 'Turno pendiente', '1' => 'Turno confirmado', '2' => 'Retiro finalizado', '-1' => 'Turno rechazado');
									...
									if(empty($row['vd_ini_real']) && !empty($row['link']) && $estado == 'Turno confirmado'){ 
									...
									if(tienePerfil(array(19,29))){ 
										$query = "EXEC Pallets_motivo_email {$row['vi_id']}";
										$msg_motivo_mail = $objSQLServer->dbGetAllRows($objSQLServer->dbQuery($query),3);
										if($msg_motivo_mail && is_array($msg_motivo_mail)){
											foreach($msg_motivo_mail as $row_motivo_mail){
												$estado.= " &nbsp;<a href=\"javascript:msgMotivoMail({$row['vi_id']}, '{$row['vi_codigo']}');\" class=\"link\">{$row_motivo_mail['descripcion']}</a>";
											}
											$estado.= " <span id='rueda-carga'></span>";
										}
									}
								}
								else{
									$aux = array(NULL => 'Sin documento', '0' => 'Sin documento', '1' => 'Pendiente de verificación', '2' => 'Acreditado', '3' => 'Asignado', '4' => 'Intercambiado', '-1' => 'Rechazado');
									$estado = (isset($aux[$row['vd_checked_function']]) ? $aux[$row['vd_checked_function']] : '--');
							...
							<?php }
							elseif($filtros['filtrar_rows']){
								if(!$_REQUEST['viewAll'] && count($arrViajes) > 29){
									$link_replace =	$_SERVER['REQUEST_URI'].'&viewAll=1'; //$('#hidSeccion').val($('#hidSeccion').val()+'&viewAll=1')
									$action = '?c='.$_POST['hidSeccion'].'&viewAll=1';
									$link_replace = isset($_POST['hidId']) ? "javascript: $('#frm_'+$('#hidSeccion').val()).attr('action','{$action}'); enviar('filtroFecha','{$_POST['hidId']}');" : $link_replace;
								?>
								<tr class="<?=$fila == 'filaPar'? 'filaImpar' : 'filaPar'?> tr-last">
									<td class="td-last" colspan="<?=$colspan?$colspan:7?>" align="center" style="font-size:11px; height:30px; line-height:30px;font-weight:bolder !important;">
										<center><?=idiomaHTML(str_replace('[LINK]',$link_replace,$lang->message->interfaz_generica->msj_limite_registros))?></center>
									</td>
								</tr>
								<? }
							}
