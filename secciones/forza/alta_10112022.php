...
<script type="text/javascript">
	...
	arrLang['msj_viaje_fecha_anterior'] = '<?=(($seccion != 'entregasforza' && $seccion != 'retirosforza') ? $lang->message->msj_viaje_fecha_anterior : '')?>';
...
<input type="hidden" id="selectdefault" value="true"/>
    	...
    	<tr>
        	<td style=" padding:10px;">
            	<?php if($tieneMovilAsignado){?>
					...
					<?php if($idViaje && $permisoEliminar && !tienePerfil(28)){ ?>									
                ...
                <fieldset class="gum_top2">
                    <select name="dador" class="no_margin float_l mitad_largo">
                        <option value="" class="placeholder">Fabricante</option>
                        <?php foreach($fabricante as $item){?>
                                    <option value="<?=$item['cl_id']?>" <?=$arrViaje['vi_dador'] == $item['cl_id'] ? 'selected="true"':''?> ><?=$item['cl_razonSocial']?></option>
                                <?}
                            ?>
                        </select>
                    </select>
                ...
                <div id="DatosCamposOpcionales" class="clear" style="display:none;">
                    ...
                    <?php if(isset($cbo_origen)){?>
                    <fieldset class="gum_top2">
                        <select name="origen_viaje" class="no_margin float_l mitad_largo">
                            <?php
                                foreach($cbo_origen as $item){?>
                                    <option value="<?=$item['re_id']?>" <?=$arrViaje['origen_viaje'] == $item['re_id'] ? 'selected="true"':''?> ><?=$item['re_nombre']?></option>
                                <?}
                            ?>
                        </select>
                    </fieldset>
            ...
            <td width="100%">
            	<div class="solapas gum clear" style="overflow:hidden;"><!-- height:92%;-->
                    ...
                    <?php if($historial[0] && tienePerfil(array(19,27,29)) && $seccion != 'retirosforza'){?>
                    ...
                    <div id="viajes-listado" class="contenido clear" style="height:100%">
                        ...
                        <fieldset class="gum_top">
                            <?php if($type != 'retiros' || $type == 'retiros' && $operacion == 'alta'){?>
                        ...
                        <table class="listado-viajes bottom-rows">
                            <thead>
                                <tr>
                                    ...
                                    <?php if(tienePerfil(array(19,29,27,28))){?>
                                    <td>
                                    ...
                                    <td>
                                    	<center>
                                        	<span class="campo1">Vencimiento</span>
                                        </center>
                                    </td>
                                    <td>
                                    	<center>
                                        	<span class="campo1">Estado</span>
                                        </center>
                                    </td>
                                    <?php }?>
                                    <!--<td>
                    ...
                    <?php if($historial[0] && tienePerfil(array(19,27,29))){?>
                    <div id="viajes-historico" class="contenido clear" style="height:100%; display:none">
                        ...
                        <td><?=encode($item['usuario'])?></td>
                        <td style="text-align:left"><?=encode($item['descripcion'])?></td>
                                </tr>
							<?php }?>
                            </tbody>
                        </table>
                    ...
                    <div id="viajes-cotizaciones" class="contenido clear" style="height:100%; display:none">
                        <?php if(tienePerfil(array(19,27,29))){//--Fabricante?>
                        <table class="listado-viajes bottom-rows">
                        <thead>
                            <tr>
                                <td width="150"><span class="campo1"><?=$lang->system->fecha?></span></td>
                    ...
                    <div id="viajes-pod" class="contenido clear" style="height:100%; display:none">
                        ...
                        <?php if($POD){
                            ...
                            <tr>
                                ...
                                <td class="td-last" style="text-align:left"><?=encode($item['sl_descripcion'])?></td>
                            </tr>
                        <?php }
                        }else{?>
                    ...
                    <!-- Fin. POD -->
                    <!-- Ini. Hoja de Ruta -->
                    <?php if(isset($valeselectronicos)){?>
                        <div id="viajes-pod" class="contenido clear" style="height:100%; display:none">
                        <table class="listado-viajes bottom-rows">
                        <thead>
                            <tr>
                                <td width="90%"><span class="campo1"><?=$lang->system->descripcion?></span></td>
                                <td class="td-last"></td>
                            </tr>
                            </thead>
                            <tbody>
                            <?php 
                            if($valeselectronicos){
                                foreach($valeselectronicos as $item){?>
                                    <tr>
                                        <td><?=encode($item['descripcion'])?></td>
                                        <td class="td-last">
                                            <?php if($_SESSION['cargamanualok']){?>
                                            <input type="checkbox" id="chk_<?=$item['st_id']?>" name="chkId[]" value="<?=$item['st_id']?>" style="display:none">
                                            <center><a href="javascript:;" onclick="javascript:enviarModificacion('baja','<?=$item['st_id']?>')"><img src="imagenes/cerrar.png" /></a></center>
                                            <?php }?>
                                        </td>
                                    </tr>
                                <?php }?>
