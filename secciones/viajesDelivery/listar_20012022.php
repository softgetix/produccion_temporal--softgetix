...
$(document).ready(function() {
    ...
    if (google.visualization != undefined) {
		...
		<?php if(count($grafic['data1'])){?>	
		i++;
		titulo[i] = '<?=$grafic['title1']?>';
		ide[i] = 'grafico1';
		cant[i] = <?=count($grafic['data1'])?>;
		var data1 = google.visualization.arrayToDataTable([<?=$grafic['data1']?>]);
		drawChartTorta(data1,titulo[i],ide[i]);
		<?php }?>
		
		<?php if(count($grafic['data2'])){?>				
		i++;
		titulo[i] = '<?=$grafic['title2']?>'; 
		ide[i] = 'grafico2';
		cant[i] = <?=count($grafic['data2'])?>;
		var data2 = google.visualization.arrayToDataTable([<?=$grafic['data2']?>]);
		drawChartBarras(data2,titulo[i],ide[i]);
		<?php }?>
		
		<?php if(count($grafic['data3'])){?>
		i++;
		titulo[i] = '<?=$grafic['title3']?>';
		ide[i] = 'grafico3';
		cant[i] = <?=count($grafic['data3'])?>;
		var data3 = google.visualization.arrayToDataTable([<?=$grafic['data3']?>]);
		drawChartLineas(data3,titulo[i],ide[i]);
		<?php }?>
		
		<?php if(count($grafic['data4'])){?>
		i++;
		titulo[i] = '<?=$grafic['title4']?>';
		ide[i] = 'grafico4';
		cant[i] = <?=count($grafic['data4'])?>;
		var data4 = google.visualization.arrayToDataTable([<?=$grafic['data4']?>]);
		drawChartBarras(data4,titulo[i],ide[i]);
		<?php }?>

        <?php if(count($grafic['data5'])){?>				
		i++;
		titulo[i] = '<?=$grafic['title5']?>'; 
		ide[i] = 'grafico5';
		cant[i] = <?=count($grafic['data5'])?>;
		var data5 = google.visualization.arrayToDataTable([<?=$grafic['data5']?>]);
...
<center>
    <?php if(count($grafic['data1'])){?><div id="grafico1" style="width: 18%; height:259px; display:inline-block"></div><?php }?>
    <?php if(count($grafic['data2'])){?><div id="grafico2" style="width: <?=(!count($grafic['data1']) && count($grafic['data4']))?26:35?>%; height:259px; display:inline-block"></div><?php }?>
    <?php if(count($grafic['data4'])){?><div id="grafico4" style="width: 35%; height:259px; display:inline-block"></div><?php }?>
    <?php if(count($grafic['data3'])){?><div id="grafico3" style="width: 25%; height:259px; display:inline-block"></div><?php }?>
    <?php if(count($grafic['data5'])){?><div id="grafico5" style="width: 18%; height:259px; display:inline-block"></div><?php }?>
</center>
<span class="clear" style="height:<?=(count($grafic['data1']) || count($grafic['data2']) || count($grafic['data3']) || count($grafic['data4']))?'20px':'0px'?>;"></span>
<?php if(!tienePerfil(array(8,12)) && $grafic['stats']){?>
<center>
	<div class="stats">
        <span>Detecci&oacute;n de entregas por GPS <strong><?='('.$grafic['stats']['en_destino']['detectados'].'/'.$grafic['stats']['en_destino']['total'].') '.$grafic['stats']['en_destino']['promedio'].'%'?></strong></span>
        <span>Moviles asociados a entregas Reportando <strong><?='('.$grafic['stats']['moviles']['reportando'].'/'.$grafic['stats']['moviles']['total'].') '.$grafic['stats']['moviles']['promedio'].'%'?></strong></span>
        <!--
        <span>Detecciones en Origen <strong><? //='('.$grafic['stats']['en_origen']['detectados'].'/'.$grafic['stats']['en_origen']['total'].') '.$grafic['stats']['en_origen']['promedio'].'%'?> </strong></span>
        -->
    </div>
</center>
<?php }?>
                    ...
                    <table class="listado-viajes" width="100%" height="100%">
						<thead>
							<tr>
								...
								<td colspan="2">
                                	<?=$objFiltroCol->label($col['movil'],$lang->system->movilsemi,'campo1')?>
                                    <span class="campo2"><?=$lang->system->conductor?></span>
                                </td>
								<td>
                                	<?=$objFiltroCol->label($col['referencia'],$lang->system->rutaviaje,'campo1')?>
                                    <span class="campo2"><?=$lang->system->Fechacarga?></span>
                                </td>
								<td>
						...
						foreach($arrViajes as $k => $row){
                            ...
                            <tr class="<?=$fila?> <?=($verContenido && !$verSubContenido)?'':((!$verContenido && $verSubContenido)?'destinos_'.$row['vi_id'].' row_viajes':(!$verSubContenido?'entrega_'.$row['vd_id'].' destinoEntrega_'.$row['vi_id']:''))?> <?=($row['vi_finalizado'])?'viaje-finalizado':''?>" <?=(!$verContenido /*|| !$verSubContenido*/)?'style="display:none"':''?>>
								...
								<td>
                                	<a href="javascript:;" class="float_r <?=($_SESSION['idEmpresa'] == 4835 || $_SESSION['idAgente'] == 4835) ? 'setNewcarArauco' : 'setNewcar' ?> <?=$verContenido?'firts':''?>" title="<?=$lang->system->asignar_conductor_movil?>"
