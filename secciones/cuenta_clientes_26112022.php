                    	...
                    	<center>
							...
							<?php } else{
								if(tienePerfil(5) && !empty($item['cl_email'])){
									$ide = 're-email-'.$i; ?>									
									<a href="javascript:CreateAccount(<?=$item['cl_id']?>, '<?=$ide?>'); SendMailResetPass('<?=$item['cl_email']?>','<?=$_SESSION['DIRCONFIG']?>','<?=$ide?>');" title="<?=$lang->system->title_cambio_clave?>" id="<?=$ide?>"><img src="imagenes/send_mail.png" /></a>
								<?php }
