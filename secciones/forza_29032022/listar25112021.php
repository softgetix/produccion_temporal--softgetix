...
<?php if(($type == 'entregas' || $_GET['cargamanualok'] == 1)&&  1==2 &&  tienePerfil(27)){?>
...
<? if (($type == 'entregas' || $_GET['cargamanualok'] == 1)  && tienePerfil(27)) { ?>


    <p><a href="javascript:;" onclick="enviar('alta');" class="button colorin" style="margin-top:5px; padding:8px 20px;"><?=strtoupper($lang->botonera->nueva_tarea)?></a></p>
                    ...
                    <table width="100%" height="100%">
						...
						foreach($arrViajes as $k => $row){
							...
							<tr class="<?=$fila?> <?=(!$verContenido)?'destinos_'.$row['vi_id'].' row_viajes':''?> <?=($row['vi_finalizado'])?'viaje-finalizado':''?>" <?php if(!$verContenido){?>style="display:none" <?php }?>>
								...
								<? 
								if ($type != 'entregas') { 
									$aux = array('0' => 'Rechazado', '1' => 'Confirmado');
