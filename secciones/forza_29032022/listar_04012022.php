...
<a href="javascript:;" onclick="enviar('alta');" class="button colorin" style="margin-top:5px; padding:8px 20px;"><?=strtoupper($lang->botonera->nueva_tarea)?></a>
	<br />
</fieldset>
...
<table width="100%" height="100%">
 ...
 <input type="text" name="buscar" id="buscador_viaje" class="buscar float_l" onkeyup="javascript:getBuscar(event, this.value)" title="<?=$lang->system->enter_buscar?>"  placeholder="<?=$lang->system->buscar_tarea?>" value="<?=$_POST['buscar']?>" autocomplete="off"/>
                   <? if ($type == 'entregas') { ?>
		    <a class="float_l margin_l button <?=($diffFechas==30 || $_POST['hidId'] == 'ultimo_mes')?'colorin':''?>" style="margin-top:5px;" onclick="javascript:enviar('filtroFecha','ultimo_mes');" href="javascript:;"><?=$lang->system->reporte_ultimo_mes_vale?></a>
                    <a class="float_l margin_l button <?=($diffFechas==0 || ($diffFechas!=30 && $_POST['hidId'] != 'ultimo_mes'))?'colorin':''?>" style="margin-top:5px;" onclick="javascript:enviar('filtroFecha','hoy');" href="javascript:;"><?=$lang->system->reporte_tiempo_real_vale?></a>
						...
						foreach($arrViajes as $k => $row){
							...
							<tr class="<?=$fila?> <?=(!$verContenido)?'destinos_'.$row['vi_id'].' row_viajes':''?> <?=($row['vi_finalizado'])?'viaje-finalizado':''?>" <?php if(!$verContenido){?>style="display:none" <?php }?>>
								...
								if ($type != 'entregas') { 
									...
									$estado = isset($aux[$row['vd_checked']]) ? $aux[$row['vd_checked']] : 'Pendiente';
									...
									if($_SESSION['cargamanualok'] == 1){
										...
										$estado.= " &nbsp;<a href=\"javascript:mostrarPopupUrlExterna('".$row['link']."&forzar=1',500,550);\" class=\"link\">Admin Viaje</a>";
									...
									$aux = array(NULL => 'Sin documento', '0' => 'Sin documento', '1' => 'Pendiente de verificación', '2' => 'Acreditado', '-1' => 'Rechazado', '3' => 'Asignado', '4' => 'Intercambiado');
									$estado = isset($aux[$row['vd_checked']]) ? $aux[$row['vd_checked']] : '--';
									if($row['vd_checked'] === NULL || $row['vd_checked'] === 0 || $row['vd_checked'] === '-1' ){ 
