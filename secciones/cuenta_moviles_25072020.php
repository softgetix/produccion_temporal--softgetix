        ...
        <?php if($arrEntidades){
            foreach($arrEntidades as $i => $item){
                ...
                <tr class="<?=$class?> <?=((count($arrEntidades) - 1)==$i)?'tr-last':''?>  <?=$item['mo_borrado']?'row_inactivo':''?>">
                    ...
                    <td><?=$item['tag']?></td>
                    <td><center><a href="javascript:;" onclick="javascript:changeDistance(this.id, <?=$item['id']?>);" id="changeDistance<?=$item['id']?>" class="iconDistancia <?=($item['un_tiempo'] < 80) ? 'dist1m' : 'dist25m'?>"></a></center></td>
                    <?php }?>
