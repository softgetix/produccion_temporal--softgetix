...
if(SITE != 'MOBILE'){
	if(esDispositivo()){
		...
		elseif(strpos($_SERVER['REQUEST_URI'], '/intraate') !== false){
			if(!$_POST){
				include('index_INTRAATE.php');
				exit;
			}
		} 
		elseif(strpos($_SERVER['REQUEST_URI'], '/palletswap') !== false){
			if(!$_POST){
				include('index_PALLETSWAP.php');
				exit;
			}
		} 
		else{
