...
$(function() {
	...
	//reloadComboTipoMovil('hide',$("#temp_transportista").val()); //--Se comenta porq en la carga invoca a este metodo sin transportista y no se visualiza desde el front..
...
function reloadComboTransportista(){
	...
	if(typeof($('#selectdefault').val()) == 'undefined'){//--No conservar primer opción del combo "Seleccione" cuando se define que seleccione por defecto el primer transportista
	...
	if(typeof($('#selectdefault').val()) != 'undefined'){
		if($(selectdefault).val() == 'true' && $('#temp_transportista').val() == ""){//--Al selecciona por defecto la primera opción del combo, se recarga los demás combos.
			$(ideCombo+' option:eq(0)').attr('selected','selected');
			$(ideCombo).trigger('onchange');
		}	
	}
}
...
function reloadComboConductor(id_transportista){
	...
	if(typeof($('#selectdefault').val()) == 'undefined'){//--No conservar primer opción del combo "Seleccione" cundo se define que seleccione por defecto el primer conductor
		$(ideCombo).append('<option value="" class="placeholder">'+txtSelect+'</option>');
	}
...
function onChanges(ide, opcion){
	...
	else if(opcion == 'comboFabricante'){
		$('#temp_fabricante').val(ide.value);
	}
	else{
		validarPallets();
	}
