...
function validarCliente($get){
	...
	elseif(isset($get['config']) || isset($_SESSION['DIRCONFIG'])){
		...
		switch($config){
			...
			break;
			case 'etaartni':
				$_SESSION['DIRCONFIG'] = 'intraate';
				define('SITE', 'INTRAATE');
				define('LOGO', 'logo_agentes_adt.jpg');
				define('FAVICON', 'faviconADT.ico');
				define('TITLE', 'Intra ATE');
				define('FOOTER', true);
			break;
			case 'tdaeta':
			...
			case 'pawstellap':
				$_SESSION['DIRCONFIG'] = 'palletswap';
				define('SITE', 'PALLETSWAP');
				define('LOGO','logo_ps.jpg');
				define('TITLE', 'Pallet Swap');
				define('FOOTER', false);
				define('MENU_REDUCIDO',false);
				define('FAVICON', 'redpalletswap.ico');
			break;
