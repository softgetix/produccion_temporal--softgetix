...
switch($_SESSION['idPerfil']){
	...
	case 37:		##-- EQUIPO forza --##
		...
		$arrNavBar = array(
			...
			$arrMenu[71] => '',		//moduloSeguridad
			$arrMenu[2] => '' ,		//Clientes y Stock
			$arrMenu[45]=> '' , // generador de informes
			$arrMenu[8] => ''	//cuenta
		...
		array_push($arrPermisos,'entregasforza');
		array_push($arrPermisos,'retirosforza');
		$arrPermisos = quitarPermisos('moviles_semi');
	...
	case 20: 	##-- Premium -> Satelital --##
		...
		$arrPermisos = asignarPermisos($arrNavBar);
	break;
	case 30: ##-- Premium -> Premium-Seguridad --##
		$paginaDefecto = $arrMenu[3];
	...
	break;
	case 32:		##-- ADT -> Administrador --##
		if(ES_MOBILE){
            $paginaDefecto = $arrMenu[72];
			$arrNavBar = array(
				$arrMenu[72] => '' //whatsapp
		...
		if(!ES_MOBILE){
			...
			array_push($arrPermisos,$arrMenu[72]);	//whatsApp
           
		}
	break;
	case 33:		##-- ADT -> Administrador --##
		if(ES_MOBILE){
            $paginaDefecto = $arrMenu[72];
			$arrNavBar = array(
				$arrMenu[72] => '' //whatsapp
