...
<head>
    ...
    <link type="text/css" rel="stylesheet" href="css/estilosDefault.css?2"/>
    ...
    <script type="text/javascript" src="js/funciones.js?18"></script>
  ...
  <script type="text/javascript" src="js/jquery.playSound.js"></script>
	...
	if(isset($extraJS) && is_array($extraJS)){
		foreach ($extraJS as $url){
			if(is_file($url)){?>
        		<script type="text/javascript" src="<?=$url?>?18"></script>
            <?php }?>    
    	<?php }
	}?>
	...
	<?php if(!isset($sinDefaultJS)){
		...
		if(is_file($urlInclude)){?>
        	<script type="text/javascript" src="js/<?=$seccion?>Funciones.js?15"></script>
        <?php }?>
    <?php } ?>
</head>
<div style="position:absolute; top:0px; left:0px; width:100%; height:100%; z-index:99999; background:#000000; display:none; text-align:center;" id="fondo_preloader">
    ...
    <input type="hidden" id="loggedUserId" value="<?=$_SESSION["idUsuario"] ?>" />
